...
namespace Reflectis.CreatorKit.Worlds.Installer.Editor
{
    public static class BreakingChangesSolver2025_4
    {
...
#if REFLECTIS_CREATOR_KIT_WORLDS_PLACEHOLDERS
        private const string progressTitle = "Creator Kit update v2025.3 -> v2025.4";
        ...
        private static void ReplaceInteractablePlaceholder()
        {
            ...
            if (canceled)
            {
                EditorUtility.DisplayDialog("Canceled", "Creator kit update canceled: the project has been only partially migrated. Run the update routine again to complete it.", "OK");
            }
            else if (failures.Count > 0)
            {
                EditorUtility.DisplayDialog("Warning", $"Creator kit update completed with {failures.Count} error(s):\n\n{string.Join("\n", failures)}\n\nSee the console for details.", "OK");
            }
            else
            {
